...
def drawBoard(board, screen):
    for i in range(len(board)):
        for j in range(len(board[i])):
            p.draw.rect(screen, ascii_to_color[board[j][i]], (60 * j, 60 * i, 60 - 2, 60 - 2), border_radius=8)
...
def fallBoard(board, board_with_underscores, screen):
    ...
    while len(fall_list):
        ...
        for i in range(len(board_with_underscores)):
            for j in range(len(board_with_underscores[i])):
                if [i, j] not in [fall_list[k][0] for k in range(len(fall_list))]:
                    #pass
                    #print('I draw static!')
                    p.draw.rect(screen, ascii_to_color[board_with_underscores[i][j]], (60 * i, 60 * j, 60 - 2, 60 - 2), border_radius=8)
        ...
        for i in range(len(fall_list) - 1, -1, -1):
            p.draw.rect(screen, ascii_to_color[fall_list[i][2]],
                        (60 * fall_list[i][0][0], 60 * fall_list[i][0][1], 60 - 2, 60 - 2), border_radius=8)
            ...
            fall_list[i][0][0], fall_list[i][0][1] = fall_list[i][0][0], fall_list[i][0][1] + 0.5
            p.display.flip()
        time.sleep(1)
